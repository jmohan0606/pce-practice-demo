...
## 2026-08-19 · Round 9 tasks 1+2 · Tier provenance rides the envelope as graph_mode, not mode
Context: Task 2 says "include served_by_tier and mode in the envelope", but the Round 3 shape/rows envelope already uses the key `mode` (shape|rows) on shape-capable queries. Overloading it would break the one contract the spec forbids breaking.
Decision: The envelope keys are `served_by_tier` + `graph_mode` (the GRAPH_CLIENT_MODE that served the request), additive on every return. In mock/local mode the MockGraphClient serves directly (no tiered dispatch stamps a tier), so run_catalog_query normalises `served_by_tier` to 4 when the raw result says mode=mock. The guard raises RuntimeError (mirroring the upsert guard's type), not CatalogError — CatalogError means "bad request", and a tier-4 fallback in real mode is an infrastructure failure a caller must never catch as a validation problem.
Reversible: yes

## 2026-08-19 · Round 9 task 3 · rule_evaluation_rows params renamed vertex_type/key_id; columns is app-layer projection
Context: The GSQL twin's parameter names must match the catalog entry exactly, and `vertex` / `key` risk GSQL keyword collisions (VERTEX is a type keyword). GSQL also cannot select attributes by a runtime name, so a `columns` parameter cannot project server-side.
Decision: The internal entry's params are `vertex_type` / `key_id` (all callers are in-repo: the evaluator and lookups.py). `columns` is accepted by the twin for signature parity but applied at the app layer; the twin's job is server-side ROW filtering (month/advisor/key in the WHERE clause), which is where the 12M-row scaling problem lived. The twin covers 14 vertex types (GRAIN_VERTICES union + the lookup vertices the converted sites use) as WHERE-guarded single-hop SELECTs per the reference/v2 EXAMPLE_query pattern; any other vertex_type returns empty sets, while the Python mock accepts any schema vertex — the mock is a superset, and every plan-reachable and app-used vertex is in the twin's set.
Reversible: yes

## 2026-08-19 · Round 9 task 9 · Conversions prefer the generic vertex fetch over the audit's EXT verdicts
Context: The audit's EXT verdicts (extend accounts_for_month / team_members / pce_dashboard_advisors) would require editing GSQL twins that sit among the 52 files being repaired in the client environment — the exact two-hands-one-file collision the operator forbade. The spec itself prefers "an existing query or a generic vertex-fetch entry over inventing a new named query".
Decision: EXT- and B-verdict sites convert onto the internal rule_evaluation_rows generic fetch (via app/graph/queries/lookups.py) or onto the three audit-named NEW queries; NO existing query was extended and no existing .gsql touched. Consequence: the client install set grows by exactly four files, and the pce_dashboard_* twins (which were never in docs/tigergraph/queries/) remain a recorded gap — the dashboard router also still bypasses run_catalog_query, so the Part A guard does not cover it (carried in ROUND_9_COMPLETE.md).
Reversible: yes (an EXT can be applied later once the client-side file repair settles)

## 2026-08-19 · Round 9 task 5 · Worked-example coherence test is figure-set equality; clearing is the default
Context: Task 5 offers regenerate-or-clear. Deterministic regeneration of arbitrary prose is not possible without an LLM call, and an LLM rewrite of client-facing published text on a threshold edit is a new risk, not a fix.
Decision: The worked example substitutes only when the OLD threshold is the only numeric figure it names (figures = standalone $-amounts/numbers; tokens fused to letters like 9R are not figures); otherwise it is cleared and worked_example_review_note marks it for a human rewrite. The note persists across further threshold edits until an example is written. The statement keeps plain substitution — it declares the threshold rather than reasoning about examples.
Reversible: yes

## 2026-08-19 · Round 9 task 10d · empty_kind separates true-zero diagnostics from structural caveats
Context: Setting empty_reason on EVERY zero-match evaluation (task 10d) collided with account_lifecycle_counts, whose notes field surfaces empty_reason as "this count is qualified" — a true zero ("ran, matched nothing") would have rendered as a caveat on every advisor summary (observed in the identity diff, 18/19 → fixed → 19/19).
Decision: The evaluator sets empty_kind (no_population | trigger_not_met) alongside the new reasons; consumers surfacing qualification notes skip results with empty_kind, diagnostic surfaces (rule preview) show them. Structural reasons (the baseline guard) stay unmarked and keep flowing into notes.
Reversible: yes

## 2026-08-19 · Round 9 task 7 · Guard patterns are receiver-narrowed where the bare form floods false positives
Context: The spec lists `\.store\b` and `\.load(` verbatim, but `\.store\b` matches every `from app.rules.store import …` / logger name, and `\.load(` matches json.load( and the SQLite persistence load — dozens of false positives would force a fat baseline, which defeats the ratchet.
Decision: `.store` is checked on a string-literal-stripped copy of the line and never on import lines; `.load(`/`.statistics(` require a store-suffixed receiver (`store.load(` / `fstore.statistics(`); out/inbound/out_ids/in_ids keep the spec's bare forms (zero false positives in the tree). The six-read evasion probe fails on all 7 lines. The ratchet self-tightens by rewriting its own BASELINE dict (concatenated marker so its own source never matches the substitution pattern).
Reversible: yes

## 2026-08-19 · Round 10 task 1 · Projection rewrite PLUS a strict one-query normalisation — and why both
Context: Task 1 prefers rewriting the twin in projection form over adding normalisation. Two things a static GSQL projection cannot do: apply the DYNAMIC `columns` parameter (GSQL cannot reference an attribute by a runtime name), and control TigerGraph's transport shape (every PRINT of a vertex set — bracketed or not — arrives as one object per PRINT with the projected attributes nested under `attributes`).
Decision: The twin is rewritten in the convention's projection form (every branch prints bracketed AS aliases including `<primary_id> AS __vertex_id`), AND run_catalog_query gains `_normalize_rule_evaluation_rows`, scoped to this ONE query: it unwraps the per-PRINT wrappers, lifts `attributes`, REFUSES any row lacking `__vertex_id` (never synthesising it from v_id — a bare-PRINT regression must stay loud, which is what keeps the contract test failable), and applies `columns` for every tier (the mock impl's private `_project` was deleted so there is exactly one projection implementation). The spec's objection to normalisation — future queries silently bypassing a generic shim — does not apply to a per-query step; every other query keeps the flat convention.
Reversible: yes

## 2026-08-19 · Round 10 task 4 · LOCAL-COMPUTE catalog entries: the local tier is CORRECT for Python-computed queries
Context: Routing rules_evaluate_plan and the pce_dashboard_* queries through run_catalog_query would trip the Round 9 tier-4 refusal — but for these the local tier is right BY DESIGN: they are Python computations (the evaluator; dashboard shaping) whose row reads go through guarded tiered queries underneath.
Decision: CATALOG entries can declare `local_compute: True` (all five are also `internal`). run_catalog_query dispatches them straight to the registered implementation — the tiered chain is never asked (no installed GSQL exists by design, so probing tiers 2/3 would only add latency and log noise) and the tier-4 refusal does not apply; the envelope carries `local_compute: True` for honesty. The pce_dashboard implementations were rewired to read rows through rule_evaluation_rows/lookups, so in real mode their payloads are computed over TigerGraph rows — the guard protection lives at the row source, which is where the data is. Consequence: pce_dashboard_months/advisors need NO GSQL twins (verify 12's "callers use queries that do" resolution), and C6-1 re-pins at 55 (49 visible + 6 internal).
Reversible: yes

## 2026-08-19 · Round 10 task 2 · advisor_nnm added as the 15th supported vertex; the other 16 stay raising
Context: The spec forbade blanket-adding 17 branches "unless the evaluator genuinely needs them" and asked which types rules actually target. Census over every stored plan: 7 types — 6 already supported plus phx_dm_pce_advisor_nnm (NNM_AWARD_THRESHOLD, PUBLISHED and active in RSV_v19, resolves the /nnm threshold; plus two NEEDS_DATA drafts).
Decision: phx_dm_pce_advisor_nnm joins RULE_EVALUATION_VERTICES and the twin (15 branches) — without it, Task 2's own fix would make real-mode evaluation of a genuinely published rule raise. No other type is added; an extractor proposing a plan on an unsupported vertex now fails loudly at evaluation with the supported list named, which is the correct conversation to have per rule rather than a silent zero.
Reversible: yes

## 2026-08-19 · Round 10 task 6a · Pushdown is per-parameter (reason_cd), never generic
Context: "Push the plan's filters down where the plan expresses them as simple attribute predicates" — but GSQL cannot reference an attribute by a runtime name, so a generic (field, op, value) pushdown parameter is inexpressible in the twin.
Decision: Pushdown is an explicit whitelist: each PUSHDOWN_FIELDS name (today: reason_cd) is a declared rule_evaluation_rows parameter with a matching WHERE clause in the twin, applied where the vertex carries the attribute (the month/advisor_sid pattern). The evaluator pushes top-level AND'ed literal-equality predicates on those names and STILL re-applies them Python-side (conjunctive — same rows out, and the mock/real contract stays identical by construction). The empty-population diagnostic names any pushdown that ran, so "0 rows in scope" is never mistaken for an unfiltered scope. Extending pushdown = add the name to PUSHDOWN_FIELDS + the catalog param + the twin clause; the contract test's drift check keeps twin and constant aligned.
Reversible: yes
